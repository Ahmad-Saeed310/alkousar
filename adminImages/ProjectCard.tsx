...
export default function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <div className="overflow-hidden shadow-md border">
      {/* Project header */}
      ...
      {/* Image grid - respects each image's orientation */}
      {images.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-2">
          {images.map(img => {
            const isPortrait = img.orientation === 'portrait'
            const spanClass = isPortrait ? 'col-span-1' : 'col-span-1 sm:col-span-2'
            const ratioClass = isPortrait ? 'aspect-[3/4]' : 'aspect-video'

            return (
              <figure key={img.id} className={`${spanClass} rounded-xl overflow-hidden bg-gray-50 group`}>
                <div className={`${ratioClass} overflow-hidden`}>
                  <img
                    src={img.image_url}
